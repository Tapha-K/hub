...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-[1.35rem] bg-card py-(--card-spacing) text-sm text-card-foreground ring-1 ring-foreground/10 [--card-spacing:--spacing(4)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-[1.35rem] *:[img:last-child]:rounded-b-[1.35rem]",
        className
      )}
      {...props} />
  );
...
function CardFooter({
  ...
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-[1.35rem] border-t bg-secondary/30 p-(--card-spacing)",
        className
      )}
      {...props} />
  );
